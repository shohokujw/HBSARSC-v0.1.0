...
set.seed(1)
sim = gHBSARSC(shape="Increasing", ntot = 2000, ngroup = 100, 
...
set.seed(1)
##################################################################################################
fout = hbsarv10(y=ydata, v=vdata, w=wdata, x=xdata, z=zdata, nbasis=20, id_group=id_group, nint=500, 
                mcmc = list(nblow=10000,smcmc=10000), prior = list(),
                shape = 'Increasing', iflagCenter=iflagCenter, iflagHBsigma=iflagHBsigma)
fout$mcmctime

# load("make_figure_3_4.rdata")
...
xrange = range(xgridt)
resid  = ydata - fout$post.est$wbetam - fout$post.est$valpham

# figue 4
fname = c("a", "b", "c", "d")
fig4_idx = c(94, 97, 100, 95)
...
save.image("make_figure_3_4.rdata")